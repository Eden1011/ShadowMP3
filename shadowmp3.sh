...
#TODO: if user exits after playing song, ask them if they want to save it to list
...
elif [[ "$1" =~ ^@([^\ ])+ ]] && [ $# -eq 1 ]; then #trying to pull from library
  item=${1#*@}
  pulled=$(grep -w "$item" "${dotfile_dir}/${dotfile_saved}")
  num_of_lines=$(echo "$pulled" | grep -c '^[^ ]') #count non blank lines
  if [ "$num_of_lines" -eq 1 ]; then
    link=${pulled#*=}
    echo "pulled 1"
#    play_item "$link"
  elif [ "$num_of_lines" -eq 0 ]; then
    echo "No matches found for item ${1}."
  else
